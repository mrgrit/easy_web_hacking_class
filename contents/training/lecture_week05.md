...
**어디부터 보나(방향).** MediForum 의 게시글, 특히 공지 같은 글을 열고 브라우저에서 **소스 보기
(Ctrl+U)** 또는 개발자도구(F12)로 HTML 을 직접 본다. 화면엔 안 보이지만 **주석(`<!-- ... -->`)**
안에 누군가 깜빡 남긴 깃발이 있는지 살핀다. 이건 W03 에서 개발자도구로 정보 노출을 찾던 그 감각
그대로다.
...
**어디부터 보나(방향).** `/api/...` 로 시작하는 주소를 추측해 본다(예: 사용자 목록류). 로그인하지
않은 상태에서 그 주소를 열었을 때 JSON 데이터가 그대로 나오는지 본다. 응답 안에서 관리자 계정의
비밀 키(`api_key` 같은 필드)를 찾으면 거기에 깃발이 있다.
...
**어디부터 보나(방향).** 진료기록(medical-records 류) 주소를 찾아, 끝의 번호를 1, 2, 3… 으로
바꿔가며 열어 본다. 어떤 기록의 진단/처방란에 깃발이 숨어 있다. "막힘 없이 다음 번호가 열린다"는
사실 자체가 이미 약점의 증거다.
...
**어디부터 보나(방향).** `/api/admin/...` 류의 주소를 추측해 본다(예: 관리자 DM 목록). 인증 없이
호출했는데 관리자들의 쪽지가 줄줄이 나오면, 그 본문 중 하나에 깃발이 있다.
...
**어디부터 보나(방향).** 회원가입·로그인 후 개발자도구로 내 쿠키 `MFSID` 값을 본다. 그 형태(규칙)를
관찰하고, 관리자는 더 일찍 로그인했을 테니 **더 낮은/이른 번호**를 추측해 쿠키 값을 바꿔본다. 그
상태로 관리자 전용 경로에 들어가 깃발을 얻는다.
...
**어디부터 보나(방향).** 회원가입·로그인 후 게시글 또는 댓글 본문에 `<script>` 가 포함된 글을
올려 본다. 그런 다음 관리자가 "신고된 글을 검토"하는 경로(admin 검토 류)를 발동시키면, 스크립트가
심겨 있을 때 깃발이 나타난다. "심는다 → 관리자가 본다 → 깃발"의 흐름이 핵심이다.

```mermaid
graph TD
    REG["회원가입·로그인"]
    POST["게시글/댓글에<br/>&lt;script&gt; 심기 (저장)"]
    BOT["관리자 봇이<br/>신고 글 검토(열람)"]
    EXEC["관리자 브라우저에서<br/>내 스크립트 실행"]
    FLAG["깃발 노출"]
    REG --> POST --> BOT --> EXEC --> FLAG
...
**Q1. 깃발을 찾았는데 자꾸 "오답"이라고 떠요.** 십중팔구 **복사 실수**다. `flag{...}` 는 중괄호
`{}` 까지 통째로, 앞뒤 공백 없이, 대소문자 그대로 넣어야 한다. 줄바꿈이나 따옴표가 섞여 들어가지
않았는지 확인하라. 깃발은 "본 대로 정확히" 제출하는 게 원칙이다.

**Q2. 어떤 문제는 시작도 안 돼요.** 5·6번은 **회원가입·로그인이 먼저** 필요하다(글을 쓰거나 내
쿠키를 봐야 하니까). MediForum 에서 가입부터 하고 도전하라. 반대로 1~4번은 대체로 로그인 없이도
시도할 수 있다.
...
| 순서 | 문제 | 무엇을 배운 것 | 어디부터 보나(방향만) |
|------|------|----------------|----------------------|
| 1 | 정찰 | W03 개발자도구 | 게시글 **소스 보기(Ctrl+U)** 의 주석을 본다 |
| 2 | 인증 없는 회원 API | W04 PII 노출 | 로그인 없이 열리는 `/api/...` 의 비밀 키를 줍는다 |
| 3 | 남의 진료기록(IDOR) | W04 IDOR | 진료기록 주소의 번호를 1, 2, 3… 바꿔본다 |
| 4 | 관리자 DM 도청 | W03/W04 인증 결함 | 인증이 빠진 `/api/admin/...` 류를 찾는다 |
| 5 | 예측 가능한 세션(응용) | W03 쿠키/세션 | 내 쿠키 `MFSID` 규칙을 보고 더 이른 번호로 바꾼다 |
| 6 | 저장형 XSS(보스) | W03/W04 XSS | 글/댓글에 `<script>` 를 심고 관리자 검토를 발동시킨다 |
