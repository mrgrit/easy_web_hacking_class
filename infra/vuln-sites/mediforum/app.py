...
def init_db():
    ...
    if cur.fetchone()[0] == 0:
        seed = [
            ("admin@medi.kr",   "admin123",  "관리자",       "admin",   1, "MediForum 관리자",        "800101-1******", "010-0000-0000", "flag{n0_4uth_us3r_ap1}"),
        ...
        cur.executemany("INSERT INTO posts(author_id,title,body,tag) VALUES(?,?,?,?)", [
            ...
            (1, "[공지] 의료정보 무단 전송 금지", "본 포럼은 익명입니다. <!-- TODO(admin): 임시 메모 지우기 flag{v13w_s0urc3_r3con} -->", "notice"),
        ])
        ...
        cur.executemany("INSERT INTO medical_records(patient_id,doctor_id,diagnosis,prescription,visit_date) VALUES(?,?,?,?,?)", [
            ...
            (1,2,"[VIP 비공개] 원장 종합검진","특이소견 없음 — 기밀코드 flag{1d0r_m3d_r3c0rds}","2026-04-20"),
        ])
        cur.executemany("INSERT INTO dms(from_id,to_id,body) VALUES(?,?,?)", [
            (4,2,"선생님 처방 감사합니다."),
            (2,4,"증상 변화 있으면 바로 알려주세요."),
            (1,3,"[관리자] 서버 백업 비밀번호 공유합니다: flag{4dm1n_dm_l34k}"),
        ])
        # CTF: 예측 가능한 세션 챌린지용 — 관리자(uid=1)의 고정 세션 sess-1001 을 미리 발급해 둔다.
        cur.execute("INSERT INTO sessions(sid,user_id,created_at) VALUES('sess-1001',1,datetime('now'))")
    con.commit(); con.close()

# ----------- 세션 (V14 predictable) -----------
_SESSION_COUNTER = {"n": 2000}  # CTF: 실제 로그인은 sess-2001+; sess-1001 은 관리자 고정 세션
...
# ----------- CTF: 관리자 전용 영역 (예측 가능한 세션 챌린지) -----------
@app.route("/admin/secret")
def admin_secret():
    # 관리자(role=admin)만 접근 가능. 예측 가능한 세션(MFSID=sess-1001)으로
    # 관리자 세션을 가로채면 이 깃발이 보인다.
    me = current_user()
...
# ----------- CTF: 저장형 XSS '관리자 봇' 시뮬레이션 -----------
@app.route("/admin/review")
def admin_review():
    # 관리자(봇)가 신고된 글/댓글을 검토한다고 가정한다. 본문에 <script> 가 심겨 있으면
    # 관리자 브라우저에서 실행된 것으로 간주하고 깃발을 노출(XSS 봇 시뮬레이션).
    cur = db().cursor()
    cur.execute("SELECT body FROM posts")
    bodies = [(r["body"] or "") for r in cur.fetchall()]
    cur.execute("SELECT body FROM comments")
    bodies += [(r["body"] or "") for r in cur.fetchall()]
    if any("<script" in b.lower() for b in bodies):
        return jsonify({"xss_triggered": True,
                        "msg": "관리자가 스크립트가 심긴 글을 열람했습니다! 공격 성공.",
                        "flag": "flag{st0r3d_xss_4dm1n_b0t}"})
    return jsonify({"xss_triggered": False,
                    "msg": "아직 스크립트가 심긴 글/댓글이 없습니다. 게시글이나 댓글에 <script> 를 심어보세요."})
